  ...
  return (
    <motion.div className="sh-card sh-card-editing" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.15 }}>
      ...
      <div className="sh-card-edit-body">
        ...
        <div className="sh-field"><span className="mono flex justify-between">MQTT TOPIC SUFFIX<span style={{ color: 'var(--ink-xdim)' }}>OPTIONAL</span></span>
          <div className="flex flex-col gap-1.5 mt-1">
            <div className="flex items-center gap-2"><span className="sh-topic-tag mono">PUB</span><input value={draft.pubTopic || ''} onChange={e => set({ pubTopic: e.target.value })} placeholder={`${draft.room.toLowerCase().replace(/\s+/g, '-')}/${draft.id}/set`} style={pubErr ? { borderColor: 'oklch(0.65 0.22 25)' } : {}} /></div>
            {pubErr && <span className="mono" style={{ fontSize: 10, color: 'oklch(0.72 0.22 25)', paddingLeft: 40 }}>⚠ {pubErr}</span>}
            <div className="flex items-center gap-2"><span className="sh-topic-tag sub mono">SUB</span><input value={draft.subTopic || ''} onChange={e => set({ subTopic: e.target.value })} placeholder={`${draft.room.toLowerCase().replace(/\s+/g, '-')}/${draft.id}/state`} /></div>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="sh-card-top">
        ...
        <div className="sh-card-actions"><button className="sh-card-gear" onClick={() => setEditing(true)} title="Edit"><Icon name="gear" size={13} /></button>{device.type === 'digital' && <Toggle on={device.on} onChange={v => onUpdate({ ...device, on: v })} />}</div>
      </div>
      {device.type === 'analog' ? (<div className="sh-card-body"><AnimatedReadout value={device.value} max={max} /><Slider value={device.value} max={max} onChange={(v, isFinal) => onUpdate({ ...device, value: v }, isFinal)} /></div>) : (<div className="sh-card-body digital"><div className="sh-card-state"><span className={`sh-state-pill ${device.on ? 'on' : ''}`}><i />{device.on ? 'ACTIVE' : 'STANDBY'}</span><span className="sh-card-id mono">#{device.id}</span></div></div>)}
      {(device.pubTopic || device.subTopic) && (<div className="sh-card-topics">{device.pubTopic && <span className="sh-card-topic-chip" title={device.pubTopic}><b>PUB</b>{device.pubTopic}</span>}{device.subTopic && <span className="sh-card-topic-chip sub" title={device.subTopic}><b>SUB</b>{device.subTopic}</span>}</div>)}
    </motion.div>
  )
})
...
export function AddDeviceTile({ onClick }) {
  return (
    <motion.button className="sh-card sh-add" onClick={onClick} variants={cardVariants} whileHover={{ y: -2, scale: 1.01 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}><div className="sh-add-inner"><div className="sh-add-plus"><Icon name="plus" size={22} /></div><div className="sh-add-label">Add Device</div><div className="sh-add-sub mono">PAIR · MQTT · ZIGBEE</div></div></motion.button>
  )
}